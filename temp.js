...
function renderResults() {
  // Tabbed interface: Results | Certificates
  const currentResultsTab = state.currentResultsTab || "results";
  state.currentResultsTab = currentResultsTab;

  // Tab bar
  const tabBarHtml = `
    <div class="results-tab-bar">
      <button class="results-tab-btn ${currentResultsTab === "results" ? "is-active" : ""}" data-results-tab="results">Results</button>
      <button class="results-tab-btn ${currentResultsTab === "certificates" ? "is-active" : ""}" data-results-tab="certificates">Certificates</button>
    </div>`;
  ...
  document.querySelectorAll("[data-results-tab]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      document.querySelectorAll("[data-results-tab]").forEach((btn) => {
        btn.classList.toggle("is-active", btn.dataset.resultsTab === tab);
      });
      state.currentResultsTab = tab;

      if (tab === "results") {
        if (resultsContainer) resultsContainer.hidden = false;
        if (certificatesContainer) certificatesContainer.hidden = true;
        renderResultsResults();
      } else if (tab === "certificates") {
        if (resultsContainer) resultsContainer.hidden = true;
        if (certificatesContainer) certificatesContainer.hidden = false;
        renderResultsCertificates();
      }
    });
  });
  ...
  if (!marathonId) {
    if (resultsContainer) {
      ...
      if (certificatesContainer) certificatesContainer.innerHTML = `<div class="empty"><strong>No results logged for this race. Log a result first before uploading a certificate.</strong></div>`;
    }
    return;
  }
  ...
  // --- Results Tab ---
  if (state.currentResultsTab !== "certificates" && resultsContainer) {
    resultsContainer.hidden = false;
    certificatesContainer.hidden = true;
  }

  // --- Certificates Tab ---
  if (state.currentResultsTab === "certificates" && certificatesContainer) {
    certificatesContainer.hidden = false;
    resultsContainer.hidden = true;
  }

  // Always render both to set up the UI
  if (resultsContainer) renderResultsResults();
